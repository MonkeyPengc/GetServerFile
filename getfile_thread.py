...
## brief: a threads-based process running on server side that handles multiple clients.
...
blksz = 1024
defaultHost = 'localhost'  # local IPC by default
defaultPort = 50001
...
def client(host, port, filename):
   ...
   sock.send((filename + '\n').encode())   
   info = sock.recv(blksz)
   
   if info != b'No such file on the server.':
      ...
      sock.close()
      file.close()
      return 'Client got', filename, 'at', now()
...
def serverthread(clientsock):
   ...
   try:
      ...
      clientsock.send(b'No such file on the server.')
